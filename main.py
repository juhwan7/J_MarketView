...
# ==============================================================================
# 1. 설정 (GitHub Secrets에서 환경변수로 불러옴)
# ==============================================================================
...
async def analyze_daily_category_reports(target_date, category_name, report_list):
    try:
        ...
        system_prompt = (
            ...
            f"다음은 '{target_date}'에 발행된 [{category_name}] 섹터의 리포트 모음입니다. "
        ...
        [오늘의 {category_name} 본문 모음] 
        {combined_text}
        
        [출력 형식 가이드라인] (HTML <b> 태그 활용)
        
        <b>📊 [{target_date}] {category_name} 심층 브리핑</b>
        ...
        <b>💡 [실전 투자 적용 포인트]</b>
        (이 섹터의 내용을 바탕으로 내일 장에서 투자자가 구체적으로 어떤 스탠스를 취해야 하는지, 조심할 리스크는 무엇인지 현실적인 조언)
        """
        ...
        message = (
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"<b>📚 [J_MarketView] {category_name} 심층 분석</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n\n"
            f"{ai_text}\n\n"
            f"<i>💡 총 {len(report_list)}개의 리포트가 요약되었습니다.</i>\n"
            f"━━━━━━━━━━━━━━━━━━━━"
        )
        return message
...
async def main():
    ...
    print("🚀 [J_MarketView PRO] 시스템을 시작합니다 (GitHub Actions 모드).")

    if not os.path.exists(RECORD_FILE):
        open(RECORD_FILE, 'w', encoding='utf-8').close()
    
    with open(RECORD_FILE, 'r', encoding='utf-8') as f:
        sent_records = set(line.strip() for line in f if line.strip())

    all_reports = []
    for cat_name, url_path in categories:
        print(f"🔍 {cat_name} 최근 일주일 치 탐색 중...")
    ...
    if not reports_by_date_and_cat:
        print("📭 수집된 리포트가 없습니다.")
        return
    ...
    for target_date in sorted_dates:
        for category_name, daily_cat_reports in reports_by_date_and_cat[target_date].items():
            record_key = f"{target_date}|{category_name}"
            
            if record_key in sent_records:
                continue
                
            print(f"🔥 [{target_date} - {category_name}] 리포트 심층 요약 시작!")
    ...
    print("✅ 오늘 자 리포트 처리 및 발송을 모두 완료했습니다.")
